    ...
    with open(DATA_PATH.joinpath(file)) as fp:
        ...
        for i, line in enumerate(fp):
            ...
            if lineType == 'v':
                ...
                indices.append(list(map(getIndicy, values)))

    matrix_vertices = np.array(vertices)
...
def create_mesh_data(option):
    ...
    if option == "human":
        ...
        vertices, faces = read_obj("car.obj")
        #intensities = np.loadtxt(DATA_PATH.joinpath("car.txt"))
        intensities = np.random.rand(len(vertices))
    else:
        raise ValueError
